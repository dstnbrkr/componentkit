 ...
 */
static FBComponentLayout crossChildLayout(const FBStackLayoutComponentChild &child,
                                          ...
                                          const CGSize size)
{
  const FBStackLayoutAlignItems alignItems = alignment(child.alignSelf, style.alignItems);
  // stretched children will have a cross dimension of at least crossMin
  const CGFloat childCrossMin = alignItems == FBStackLayoutAlignItemsStretch ? crossMin : 0;
  const FBSizeRange childSizeRange = directionSizeRange(style.direction, stackMin, stackMax, childCrossMin, crossMax);
...
/**
 ...
 of the items once stretched though; FBStackPositionedLayout will do centering etc.

 Finds the maximum cross dimension among child layouts.  If that dimension exceeds the minimum cross layout size then
 we must stretch any children whose alignItems specify FBStackLayoutAlignItemsStretch.
 ...
 @param size the size of the stack layout component. May be undefined in either or both directions.
 */
static void stretchChildrenAlongCrossDimension(std::vector<FBStackUnpositionedItem> &layouts,
                                               const FBStackLayoutComponentStyle &style,
                                               const CGSize size)
{
  ...
  for (auto &l : layouts) {
    const FBStackLayoutAlignItems alignItems = alignment(l.child.alignSelf, style.alignItems);
    ...
    if (alignItems == FBStackLayoutAlignItemsStretch && fabsf(cross - childCrossMax) > 0.01) {
...
/**
 ...
 @param style the layout style of the overall stack layout
 */
static CGFloat computeStackDimensionSum(const std::vector<FBStackUnpositionedItem> &children,
                                        const FBStackLayoutComponentStyle &style)
{
  ...
  const CGFloat childSpacingSum = std::accumulate(children.begin(), children.end(),
                                                  ...
                                                  [&](CGFloat x, const FBStackUnpositionedItem &l) {
 ...
 */
static std::function<BOOL(const FBStackUnpositionedItem &)> isFlexibleInViolationDirection(const CGFloat violation)
{
  if (fabsf(violation) < kViolationEpsilon) {
    return [](const FBStackUnpositionedItem &l) { return NO; };
  } else if (violation > 0) {
    return [](const FBStackUnpositionedItem &l) { return l.child.flexGrow; };
  } else {
    return [](const FBStackUnpositionedItem &l) { return l.child.flexShrink; };
  }
}

static inline BOOL isFlexibleInBothDirections(const FBStackLayoutComponentChild &child)
{
 ...
 */
static inline BOOL useOptimizedFlexing(const std::vector<FBStackLayoutComponentChild> &children,
                                       const FBStackLayoutComponentStyle &style,
                                       const FBSizeRange &sizeRange)
{
 ...
 */
static void layoutFlexibleChildrenAtZeroSize(std::vector<FBStackUnpositionedItem> &items,
                                             const FBStackLayoutComponentStyle &style,
                                             const FBSizeRange &sizeRange,
                                             const CGSize size)
{
  for (FBStackUnpositionedItem &item : items) {
 ...
 */
static void flexChildrenAlongStackDimension(std::vector<FBStackUnpositionedItem> &items,
                                                         const FBStackLayoutComponentStyle &style,
                                                         const FBSizeRange &sizeRange,
                                                         const CGSize size,
                                                         const BOOL useOptimizedFlexing)
{
  ...
  std::function<BOOL(const FBStackUnpositionedItem &)> isFlex = isFlexibleInViolationDirection(violation);
...
/**
 ...
 stretched.
 */
static std::vector<FBStackUnpositionedItem> layoutChildrenAlongUnconstrainedStackDimension(const std::vector<FBStackLayoutComponentChild> &children,
                                                                                           const FBStackLayoutComponentStyle &style,
                                                                                           const FBSizeRange &sizeRange,
                                                                                           const CGSize size,
                                                                                           const BOOL useOptimizedFlexing)
{
  ...
  return FB::map(children, [&](const FBStackLayoutComponentChild &child) -> FBStackUnpositionedItem {
...
FBStackUnpositionedLayout FBStackUnpositionedLayout::compute(const std::vector<FBStackLayoutComponentChild> &children,
                                                             const FBStackLayoutComponentStyle &style,
                                                             const FBSizeRange &sizeRange)
{
  // If we have a fixed size in either dimension, pass it to children so they can resolve percentages against it.
  // Otherwise, we pass kFBComponentParentDimensionUndefined since it will depend on the content.
  const CGSize size = {
    (sizeRange.min.width == sizeRange.max.width) ? sizeRange.min.width : kFBComponentParentDimensionUndefined,
    (sizeRange.min.height == sizeRange.max.height) ? sizeRange.min.height : kFBComponentParentDimensionUndefined,
  };
  ...
  std::vector<FBStackUnpositionedItem> items = layoutChildrenAlongUnconstrainedStackDimension(children,
                                                                                              style,
                                                                                              sizeRange,
                                                                                              size,
                                                                                              optimizedFlexing);
