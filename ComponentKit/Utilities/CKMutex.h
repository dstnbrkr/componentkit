...
namespace CK {
  ...
  {
    Condition () {
      CK_THREAD_ASSERT_ON_ERROR(pthread_cond_init(&_c, NULL));
    }

    ~Condition () {
      CK_THREAD_ASSERT_ON_ERROR(pthread_cond_destroy(&_c));
    }
    ...
    void signal() {
      CK_THREAD_ASSERT_ON_ERROR(pthread_cond_signal(&_c));
    }

    void wait(Mutex &m) {
      CK_THREAD_ASSERT_ON_ERROR(pthread_cond_wait(&_c, m.mutex()));
    }
