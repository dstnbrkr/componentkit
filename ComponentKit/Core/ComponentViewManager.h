...
namespace FB {
  namespace Component {
    ...
    public:
      PersistentAttributeShape(const FBViewComponentAttributeValueMap &attributes)
    ...
    private:
       ...
       */
      int32_t _identifier;
      static int32_t computeIdentifier(const FBViewComponentAttributeValueMap &attributes);
    };

    struct ViewKey {
      /**
       Class of the FBComponent. Even if two different FBComponent classes have the same viewClassIdentifier, we don't
...
/** Specialize std::hash. */
namespace std {
  template<> struct hash<FB::Component::PersistentAttributeShape>
  {
    size_t operator()(const FB::Component::PersistentAttributeShape &s) const
    {
      return std::hash<int32_t>()(s._identifier);
    }
  };

  template <> struct hash<FB::Component::ViewKey>
  {
    size_t operator()(const FB::Component::ViewKey &k) const
    {
      return [k.componentClass hash]
              ^ hash<std::string>()(k.viewClassIdentifier)
              ^ std::hash<FB::Component::PersistentAttributeShape>()(k.attributeShape);
    }
  };
}
...
namespace FB {
  namespace Component {
    ...
    public:
      ...
      UIView *viewForClass(const FBComponentViewClass &viewClass, UIView *container);
    private:
    ...
    public:
      ...
      UIView *viewForConfiguration(Class componentClass, const FBComponentViewConfiguration &config, UIView *container);
    private:
    ...
    public:
      ...
      UIView *viewForConfiguration(Class componentClass, const FBComponentViewConfiguration &config);
    ...
    public:
      static void apply(UIView *view, const FBComponentViewConfiguration &config);
    };
  }
}
