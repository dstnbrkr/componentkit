...
#import "FBComponentViewAttribute.h"
...
namespace std {
  ...
  struct hash<SetterCacheKey>
  {
    ...
    {
      return FBIntegerPairHash([key.cls hash], std::hash<void *>()((void *)key.sel));
    }
  };
}
...
FBComponentViewAttribute::FBComponentViewAttribute(SEL setter) :
...
FBComponentViewAttribute::~FBComponentViewAttribute() {}

FBComponentViewAttribute FBComponentViewAttribute::LayerAttribute(SEL setter)
{
  return FBComponentViewAttribute(std::string("layer") + sel_getName(setter), ^(UIView *view, id value){
