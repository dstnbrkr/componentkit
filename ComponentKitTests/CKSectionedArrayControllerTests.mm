...
@interface FBSectionedArrayControllerTests : XCTestCase
@end

@implementation FBSectionedArrayControllerTests
{
  FBSectionedArrayController *_controller;
}

- (void)setUp
{
  [super setUp];
  _controller = [[FBSectionedArrayController alloc] init];
}
...
@interface FBSectionedArrayControllerInsertionTests : XCTestCase
@end

@implementation FBSectionedArrayControllerInsertionTests
{
  FBSectionedArrayController *_controller;
}
...
- (void)testInsertionOfMultipleObjectsInDifferentSections
{
  ...
  expectedSections.insert(2);
  Output::Items expectedItems;
  expectedItems.insert({{0, 1}, @0});
  expectedItems.insert({{1, 1}, @1});
  expectedItems.insert({{0, 2}, @2});
  expectedItems.insert({{1, 2}, @3});
  Output::Changeset expected = {expectedSections, expectedItems};
...
@interface FBSectionedArrayControllerRemovalTests : XCTestCase
@end

@implementation FBSectionedArrayControllerRemovalTests
{
  FBSectionedArrayController *_controller;
}
...
- (void)testRemovalOfMultipleObjectsInDifferentSections
{
  ...
  expectedItems.remove({{2, 1}, @2});
  expectedItems.remove({{0, 2}, @3});
  expectedItems.remove({{1, 2}, @4});
...
@interface FBSectionedArrayControllerUpdateTests : XCTestCase
@end

@implementation FBSectionedArrayControllerUpdateTests
{
  FBSectionedArrayController *_controller;
}
...
@interface FBSectionedArrayControllerEnumerationTest : XCTestCase
@end

@implementation FBSectionedArrayControllerEnumerationTest
{
  FBSectionedArrayController *_controller;
}
...
- (void)testSingleSectionEnumerationWithSingleSection
{
  ...
  [_controller enumerateObjectsInSectionAtIndex:0 usingBlock:^(id<NSObject> object, NSIndexPath *indexPath, BOOL *stop) {
    enumerated.push_back({{[indexPath item], [indexPath section]}, object});
  }];

  Enumerated expected;
  expected.push_back({{0, 0}, @0});
  expected.push_back({{1, 0}, @1});
