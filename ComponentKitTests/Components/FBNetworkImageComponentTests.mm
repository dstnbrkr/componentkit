...
typedef id (^FBTestImageDownloaderDownloadImageBlock)(NSURL *url,
                                                      id scenePath,
                                                      id caller,
...
@interface FBTestImageDownloader : NSObject <FBNetworkImageDownloading>
@end

@implementation FBTestImageDownloader
{
  FBTestImageDownloaderDownloadImageBlock _downloadImageBlock;
}

- (instancetype)initWithDownloadImageBlock:(FBTestImageDownloaderDownloadImageBlock)downloadImageBlock
{
  self = [super init];
  if (self) {
...
@interface FBNetworkImageComponentTests : FBComponentSnapshotTestCase
@end

@implementation FBNetworkImageComponentTests
...
{
  FBNetworkImageComponent *c =
  [FBNetworkImageComponent
   newWithURL:nil
   imageDownloader:nil
   scenePath:nil
   size:{}
  ...
  static FBSizeRange kSize = {{0, 0}, {50, 50}};
...
{
  FBTestImageDownloader *imageDownloader =
    [[FBTestImageDownloader alloc] initWithDownloadImageBlock:^id(NSURL *url,
                                                                  id scenePath,
                                                                  id caller,
                                                                  dispatch_queue_t callbackQueue,
                                                                  void (^downloadProgressBlock)(CGFloat),
