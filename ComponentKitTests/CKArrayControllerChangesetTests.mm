...
/**
 ...
 FBSectionedArrayController **without** having to change any of the index paths in the input commands.
 */
...
- (void)testMapThrowsOnNilReturnValueFromBlock
{
  ...
  XCTAssertThrowsSpecificNamed(input.map(mapper), NSException, NSInvalidArgumentException, @"");
}
...
- (void)testMap
{
  ...
  expectedItems.insert({1, 2}, @2);
  expectedItems.insert({0, 2}, @3);
  expectedItems.insert({15, 1}, @4);
  expectedItems.insert({5, 1}, @5);
  expectedItems.update({6, 6}, @6);
  expectedItems.update({5, 6}, @6);
  expectedItems.remove({9, 15});
  expectedItems.remove({8, 15});
...
static Output::Changeset exampleOutputChangeset(void)
{
  ...
  items.insert({{0, 2}, @2});
  items.insert({{1, 2}, @3});
  items.remove({{10, 15}, @4});
  items.remove({{9, 15}, @5});
  items.remove({{4, 16}, @6});
  items.remove({{5, 16}, @7});
  items.update({{6, 7}, @8, @9});
  items.update({{5, 7}, @8, @9});
  items.update({{3, 6}, @8, @9});
  items.update({{4, 6}, @8, @9});
...
- (void)testMapThrowsOnNonNilInsertBefore
{
  ...
  XCTAssertThrowsSpecificNamed(output.map(mapper), NSException, NSInvalidArgumentException, @"");
}
...
- (void)testMap
{
  ...
  expectedItems.insert({{0, 2}, @3});
  expectedItems.insert({{1, 2}, @4});
  expectedItems.remove({{10, 15}, @5});
  expectedItems.remove({{9, 15}, @6});
  expectedItems.remove({{4, 16}, @7});
  expectedItems.remove({{5, 16}, @8});
  expectedItems.update({{6, 7}, @9, @10});
  expectedItems.update({{5, 7}, @9, @10});
  expectedItems.update({{3, 6}, @9, @10});
  expectedItems.update({{4, 6}, @9, @10});
